...
- `scenario_ground_truth.json`
  - scenario structure plus indexed executed attacks treated as ground-truth reconstruction context
...
- `foc_readiness_report.json`
  - semantic completeness gate for the derived FOC layer before later causal reconstruction is allowed
...
Before generating later causal products such as `causal_graph.json`, the module now validates whether the derived context is semantically complete enough to support traceable reconstruction.
