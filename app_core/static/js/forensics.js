...
function guessLatestDumpFromManifest(manifest) {
  // Busca el último artifact tipo memory / lime
...
function renderArtifacts(manifest) {
  ...
  // auto-fill vol_dump_file if we find a memory dump in manifest
  const lastDump = guessLatestDumpFromManifest(manifest);
  if (lastDump) {
    // backend decide si quiere path absoluto o rel; aquí ponemos rel por defecto
...
async function acquireMemory() {
  ...
  // refresca manifest para capturar dump
...
async function analyzeMemory() {
  ...
  // refresca manifest para mostrar outputs
  try { await refreshManifest(); } catch {}
}
