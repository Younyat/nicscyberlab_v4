...
@api_bp.route("/api/openstack/flavors")
...
@api_bp.route("/api/openstack/networks")
def openstack_networks():
    conn = get_openstack_connection()
    nets = []
    for n in conn.network.networks():
        cidr = None
        for s in conn.network.subnets(network_id=n.id):
            cidr = s.cidr
        nets.append({"id": n.id, "name": n.name, "cidr": cidr})
    return jsonify({"networks": nets})


@api_bp.route("/api/openstack/security-groups")
def openstack_secgroups():
    conn = get_openstack_connection()
    return jsonify({
        "security_groups": [{"id": sg.id, "name": sg.name}
                            for sg in conn.network.security_groups()]
    })


@api_bp.route("/api/openstack/keypairs")
def openstack_keypairs():
    conn = get_openstack_connection()
    return jsonify({
        "keypairs": [{"name": k.name} for k in conn.compute.keypairs()]
    })
...
def api_openstack_instances_full():
    ...
    for server in conn.compute.servers(details=True):
        ...
        for v in server.attached_volumes:
            vol = conn.block_storage.get_volume(v["id"])
            volumes.append({
                "id": vol.id,
                "name": vol.name,
                "size_gb": vol.size,
