...
if [[ $# -lt 2 ]]; then
    echo "Usage: $0 <CASE_DIR> <DISK_RAW_ABS_PATH>"
    exit 1
fi
        ...
        if [[ -s "$PART_OUT/bodyfile.txt" ]]; then
            ...
            if [[ "$FS_TYPE" == *"Ext4"* ]]; then
                ...
                if [[ -n "$AUTH_INODE" ]]; then
                    icat -o "$off" "$DISK" "$AUTH_INODE" > "$RECOVERY_DIR/auth.log" 2>/dev/null
                ...
                if [[ -n "$BASH_INODE" ]]; then
                    icat -o "$off" "$DISK" "$BASH_INODE" > "$RECOVERY_DIR/bash_history_ubuntu" 2>/dev/null
                ...
                if [[ -n "$PASSWD_INODE" ]]; then
                    icat -o "$off" "$DISK" "$PASSWD_INODE" > "$RECOVERY_DIR/passwd" 2>/dev/null
                fi
            fi
            # -------------------------------------------------------
        else
