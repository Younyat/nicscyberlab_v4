...
def manifest_sizes(manifest: dict, case_dir: str) -> Dict[str, List[int]]:
    """
    1) Primero intenta por manifest.json (como ya hacías).
    2) Si NO hay PCAPs en manifest, cae a filesystem del CASE:
...
def evidence_quality(case_dir: str, manifest: dict) -> Dict[str, Any]:
    ...
    # red: considera también traffic_preserved aunque no esté en manifest
    network_dir = os.path.join(case_dir, "network")
    preserve_root = os.path.join(network_dir, "traffic_preserved", "full_scenario_captures")
    e1_network = os.path.isdir(network_dir) or has_prefix("network/") or bool(_walk_pcaps(preserve_root))
    ...
    return {
        ...
        "e2_max_offset_skew_ms": None,
        "e3_manifest_has_sha256": bool(sha_any),
        "e3_custody_chained_verified": None,
        "e4_primary_derived_separation": bool(e4),
    }


def _infer_pcap_events_from_files(case_dir: str, run_id: str) -> Tuple[Optional[dict], Optional[dict]]:
    """
    Si no hay pcap_start/pcap_preserved en pipeline_events.jsonl, inferimos por mtime
    dentro del CASE (traffic_preserved). Esto NO sustituye la trazabilidad real, pero
    te rellena tablas con lo que ya guardas hoy.
    """
    preserve_root = os.path.join(case_dir, "network", "traffic_preserved", "full_scenario_captures")
    pcaps = _walk_pcaps(preserve_root)
    if not pcaps:
        # fallback: cualquier .pcap bajo network/
        pcaps = _walk_pcaps(os.path.join(case_dir, "network"))
    if not pcaps:
        return (None, None)
    ...
    # Creamos "event dict" compatible con latency_s()
...
def build_run_record(case_dir: str, case_name: str, run_id: str, alerts_root: Optional[str]) -> Dict[str, Any]:
    ...
    if pcap_start is None or pcap_pres is None:
        inf_start, inf_pres = _infer_pcap_events_from_files(case_dir, run_id)
