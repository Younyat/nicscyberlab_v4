...
- PCAP start/preserved: uses explicit pcap_* events if present; otherwise falls back to traffic_capture_*;
  otherwise infers by filesystem mtime range (marked as inferred in debug fields).
"""
...
def extract_alert_invariants_from_obj(obj: dict) -> Dict[str, str]:
    ...
    # Optional: signature_id/rev if present
...
def extract_alert_invariants(case_dir: str, alerts_root: Optional[str]) -> Dict[str, str]:
    """
    Preferred: CASE/alerts/*.json (first file sorted).
    Fallback: latest alerts_store ALERTS-.../alerts.jsonl (first event).
    """
...
def read_time_sync_max_offset_ms(case_dir: str) -> Optional[float]:
    ts_path = os.path.join(case_dir, "metadata", "time_sync.json")
    obj = read_json(ts_path)
    if not obj:
        return None
    v = obj.get("max_offset_ms")
    try:
        if v is None:
            return None
        return float(v)
    ...
    try:
        with open(path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                # recompute entry_hash from entry without entry_hash field
                entry_wo = dict(entry)
...
def evidence_quality(case_dir: str, manifest: dict) -> Dict[str, Any]:
    ...
    # E1: minimal artifacts present (manifest + disk + network)
    ...
    # E2: time sync max offset ms (if exported)
    e2 = read_time_sync_max_offset_ms(case_dir)

    # E3: manifest has sha256
    sha_any = any(bool(a.get("sha256")) for a in (manifest or {}).get("artifacts", []) or [] if isinstance(a, dict))

    # E3: custody chain verified
    custody_ok = verify_custody_chain(case_dir)

    # E4: primary/derived separation (structural flag; provenance completeness is outside this script)
...
def main() -> None:
    ...
    ap.add_argument("--format", choices=["json", "jsonl", "csv"], default="json", help="Output format.")
    args = ap.parse_args()

    evidence_root = os.path.abspath(args.evidence_root)
    cases = pick_cases(evidence_root, args.limit)
    if not cases:
        output_json({"error": "no CASE-* directories found", "evidence_root": evidence_root})
        return
