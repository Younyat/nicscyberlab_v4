...
OUT_DIR="${CASE_DIR}/analysis/analyze_memory_vol3"
...
pick_symbols_dir() {
  if [[ -d "${HOME}/vol3_symbols_cache/symbols" ]]; then
    echo "${HOME}/vol3_symbols_cache/symbols"
    return 0
  fi

  if [[ -d "${CASE_DIR}/memory/symbols" ]]; then
    echo "${CASE_DIR}/memory/symbols"
    return 0
  fi

  if [[ -d "${HOME}/.cache/volatility3/symbols" ]]; then
    echo "${HOME}/.cache/volatility3/symbols"
    return 0
  fi
