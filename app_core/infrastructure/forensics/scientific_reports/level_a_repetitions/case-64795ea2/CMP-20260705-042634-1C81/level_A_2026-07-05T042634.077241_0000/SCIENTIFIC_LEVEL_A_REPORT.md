...
- Generated at: `2026-07-05T04:26:34.077241+00:00`
- Campaign ID: `CMP-20260705-042634-1C81`
- Anchor execution ID: `EXEC-0002`
- Requested dry-run repetitions: `2`
- Generated dry-run repetitions: `EXEC-0001, EXEC-0002`
- Preserved case ID: `case-64795ea2`
- Report directory: `app_core/infrastructure/forensics/scientific_reports/level_a_repetitions/case-64795ea2/CMP-20260705-042634-1C81/level_A_2026-07-05T042634.077241_0000`

## Executive scientific summary

This report audits a Level A repetition campaign over the same preserved case. It launched the same `Run Dry-Run Execution` scientific backend path **2** times against the same preserved evidence set in read-only mode, then consolidated those outputs into one auditable report. The preserved case shows **13 / 15** analysis layers with useful output, a causal recovery of **3 / 8** expected relations, and a repeatability comparison status of **Comparable With Degradation**.
...
- Case ID: `case-64795ea2`
- Scenario ID: `scn-b83dbbfb`
- Source case name: `CASE-20260705-040250`
- Summary status: `stale`
- Integrity status: `completed`
- Case-wide integrity ratio: `0.9149`
...
- story step: Case-wide manifest validation is partial (0.9149); some artifacts remain hash-unvalidated.
- story step: IDS signature name asserts a write; not confirmed at packet level (see network layer counter-evidence).
- story step: Modbus register and value precision (declared in ground truth as register=4, expected_value=30) is not confirmed by packet-level parsing; only the presence of Modbus traffic is verified here.
...
- CPR: `0.375`
- Weighted CPR: `0.3904`
- Reconstruction confidence: `0.4974`
...
- `app_core/infrastructure/forensics/evidence_store/CASE-20260705-040250/analysis/03_network/network_findings.json` | network_analysis | role: network_analysis extraction
- `app_core/infrastructure/forensics/evidence_store/CASE-20260705-040250/analysis/04_memory/memory_findings.json` | memory_analysis | role: memory_analysis extraction
- `app_core/infrastructure/forensics/evidence_store/CASE-20260705-040250/analysis/05_disk/disk_findings.json` | disk_analysis | role: disk_analysis extraction
- `app_core/infrastructure/forensics/evidence_store/CASE-20260705-040250/analysis/06_ot/ot_findings.json` | ot_analysis | role: ot_analysis extraction
- `app_core/infrastructure/forensics/evidence_store/CASE-20260705-040250/analysis/07_alerts/alert_findings.json` | alerts_analysis | role: alerts_analysis extraction
- `app_core/infrastructure/forensics/evidence_store/CASE-20260705-040250/analysis/09_timeline/unified_forensic_timeline.json` | timeline_analysis | role: timeline_analysis extraction
- `app_core/infrastructure/forensics/evidence_store/CASE-20260705-040250/analysis/10_findings/cross_layer_findings.json` | cross_layer_findings | role: cross_layer_findings extraction
- `app_core/infrastructure/forensics/evidence_store/CASE-20260705-040250/analysis/forensic_analysis_report.json` | analysis_report | role: multilayer analysis completeness and layer outputs
- `app_core/infrastructure/forensics/evidence_store/CASE-20260705-040250/analysis/visual/analysis_visual_summary.json` | analysis_visual_summary | role: layer usefulness and indexed outputs
- `app_core/infrastructure/forensics/evidence_store/CASE-20260705-040250/chain_of_custody.log` | chain_of_custody | role: custody validation and preservation linkage
- `app_core/infrastructure/forensics/evidence_store/CASE-20260705-040250/derived/evidence_support/claimability_report.json` | claimability_report | role: supported, partial, unsupported claim structure
- `app_core/infrastructure/forensics/evidence_store/CASE-20260705-040250/derived/evidence_support/counter_evidence_report.json` | counter_evidence_report | role: contradictions and counter-evidence review
- `app_core/infrastructure/forensics/evidence_store/CASE-20260705-040250/derived/evidence_support/forensic_storyline.json` | forensic_storyline | role: semantic readable storyline
- `app_core/infrastructure/forensics/evidence_store/CASE-20260705-040250/derived/evidence_support/hypothesis_support_report.json` | hypothesis_support | role: hypothesis support and claimability evaluation
- `app_core/infrastructure/forensics/evidence_store/CASE-20260705-040250/derived/executive/evidence_lifecycle_summary.json` | evidence_lifecycle_summary | role: evidence_lifecycle_summary extraction
- `app_core/infrastructure/forensics/evidence_store/CASE-20260705-040250/derived/reconstruction/causal_graph.json` | causal_graph | role: edge-level causal audit and degraded/missing relation review
- `app_core/infrastructure/forensics/evidence_store/CASE-20260705-040250/derived/reconstruction/causal_status.json` | causal_status | role: causal reconstruction status
- `app_core/infrastructure/forensics/evidence_store/CASE-20260705-040250/derived/reconstruction/reconstruction_metrics.json` | causal_metrics | role: causal recovery metrics
- `app_core/infrastructure/forensics/evidence_store/CASE-20260705-040250/derived/reconstruction/uncertainty_report.json` | uncertainty_report | role: temporal and integrity uncertainty interpretation
- `app_core/infrastructure/forensics/evidence_store/CASE-20260705-040250/manifest.json` | case_manifest | role: case identity and preserved artifact inventory
- `app_core/infrastructure/forensics/evidence_store/CASE-20260705-040250/metadata/time_sync.json` | time_sync_metadata | role: time_sync_metadata extraction
- `app_core/infrastructure/forensics/evidence_store/repetition_campaigns/CMP-20260705-042634-1C81/comparisons/comparison-207af6d0927e/comparability_report.md` | comparability_report | role: human-readable comparison report
- `app_core/infrastructure/forensics/evidence_store/repetition_campaigns/CMP-20260705-042634-1C81/comparisons/comparison-207af6d0927e/comparability_result.json` | comparability_result | role: Level A repeatability comparison status and drift metrics
- `app_core/infrastructure/forensics/evidence_store/repetition_campaigns/CMP-20260705-042634-1C81/comparisons/comparison-207af6d0927e/comparison_matrix.json` | comparison_matrix | role: execution row comparison matrix
- `app_core/infrastructure/forensics/evidence_store/repetition_campaigns/CMP-20260705-042634-1C81/level_A/EXEC-0002/analysis_repeatability_profile.json` | analysis_repeatability_profile | role: Level A repeatability metrics
- `app_core/infrastructure/forensics/evidence_store/repetition_campaigns/CMP-20260705-042634-1C81/level_A/EXEC-0002/forensic_comparison_profile.json` | comparison_profile | role: Level A execution comparison profile
- `app_core/infrastructure/forensics/evidence_store/repetition_campaigns/CMP-20260705-042634-1C81/level_A/EXEC-0002/forensic_result_card.json` | forensic_result_card | role: lightweight Level A result card
