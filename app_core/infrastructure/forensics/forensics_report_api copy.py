...
def _build_targets(artifacts: list):
    """
    Construye targets visuales SOLO a partir de evidencia primaria.
    No incluye metadata, digest, custody, manifest ni UUIDs como nombre final.
    """
    found = {}

    for a in artifacts:
        rel_path = a.get("rel_path", "") or ""
        family = a.get("family") or _classify_artifact_family(a.get("type", ""), rel_path)

        # Solo evidencia primaria
        if family not in ("memory", "disk", "network", "industrial"):
            continue
        ...
        if "plc" in p:
            ...
            # no mostrar UUID como target final
            key = "victim"
            label = "Victim"
        else:
            # si no sabemos qué es, no lo metemos
            continue
...
def _build_summary(case_dir: str):
    ...
    return {
        ...
        "targets": _build_targets(artifacts),
        "summary": {
