...
net_stats="$(awk -F'[: ]+' 'NR>2 {iface=$2; if (iface != "" && iface != "lo") {rx+=$3; tx+=$11}} END {printf "%.0f\t%.0f\n", rx+0, tx+0}' /proc/net/dev 2>/dev/null)"
...
kv net_rx_bytes "${net_rx_bytes:-not_available}"
kv net_tx_bytes "${net_tx_bytes:-not_available}"
